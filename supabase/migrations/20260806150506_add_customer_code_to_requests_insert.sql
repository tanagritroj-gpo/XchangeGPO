-- requests.customer_code ไม่เคยถูกเซ็ตค่าเลยสักแถวเดียวในข้อมูลจริง (ทั้ง submission_channel
-- 'customer_portal' และ 'csr_manual') เพราะ create_exchange_request() ไม่ได้ insert คอลัมน์นี้
-- เลยตั้งแต่แรก ทำให้รายงาน "พอร์ตลูกค้า/หน่วยงาน" ใน Manager Download Center ที่เคย join ด้วย
-- requests.customer_code ตรงๆ ได้ค่า 0 ทุกหน่วยงาน (แก้ทางแอปไปแล้วให้ join ผ่าน b2b_customer_id
-- แทน แต่ควรแก้ต้นตอที่ฐานข้อมูลด้วย ไม่ให้คอลัมน์นี้ค้างว่างต่อไปเรื่อยๆ)
--
-- แก้โดย: 1) ให้ create_exchange_request() insert customer_code จาก p_request_data ด้วย
--         (ฝั่ง TypeScript ต้องส่ง customer_code เข้ามาใน requestData คู่กัน — ดู
--         app/actions/form-actions.ts และ app/actions/staff-form-actions.ts)
--         2) backfill ข้อมูลเก่าที่ยังว่างอยู่ทั้งหมด ด้วยการ join ผ่าน b2b_customer_id ->
--         b2b_customers.organization_id -> organizations.customer_code (สาย join เดียวกับที่
--         ยืนยันแล้วว่าถูกต้องตอนแก้ downloads-export/route.ts)
...
-- backfill แถวเก่าที่ customer_code ยังว่างอยู่ ผ่านสาย b2b_customer_id -> organizations
