-- เพิ่มคอลัมน์ราคาต่อหน่วย (unit_price) ใน drug_items — value_amount ยังเก็บ "มูลค่ารวม" เหมือนเดิม
-- (คำนวณจาก qty × unit_price ฝั่ง server ใน form-actions.ts / staff-form-actions.ts เสมอ ไม่เชื่อค่าจาก client)
ALTER TABLE public.drug_items ADD COLUMN unit_price numeric;

-- อัปเดต create_exchange_request() ให้รับและบันทึก unit_price ด้วย — logic ส่วนอื่นคงเดิมทุกจุด
-- (อ้างอิงจาก supabase/migrations/20260731081734_add_province_to_create_exchange_request_rpc.sql
-- ซึ่งเป็นเวอร์ชันล่าสุดก่อนหน้านี้)
