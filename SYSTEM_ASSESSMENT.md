...
หลังจากประเมินครั้งแรก มีการแก้ไขเป็น commit บน branch `RLS_supabase` ต่อเนื่องรวม 24 ครั้ง (`9c47595` → `dc2674b`) สรุปสถานะเทียบกับ finding เดิมในเอกสารนี้ — รอบก่อนหน้า (`46467a7` → `a540fe7`) ปิด 5.8, 5.9 และ `console.log` ที่หลุดไป browser เพิ่ม พร้อมรวมเนื้อหานโยบายที่ซ้ำซ้อนเป็นแหล่งเดียว และเพิ่มความสามารถ AI chatbot ทั้งสองตัว รอบถัดมา (`bf0de0a` → `c082350`) แก้บั๊ก function calling ของ staff-chat, ปิดช่องโหว่ `b2b_customers.customer_code` ไม่เคยถูกกรอก และปิดจุด UX ที่ลูกค้าค้างอยู่หน้าลงทะเบียนหลังกดยืนยันสำเร็จ ส่วนรอบล่าสุด (`3e87473` → `dc2674b`) เพิ่มฟีเจอร์เอกสารยืนยันการลงทะเบียน (PDF + อีเมล) แล้วตามแก้บั๊กที่เจอจากการทดสอบจริงต่อเนื่องรวม 5 จุด รวมถึง `checkRateLimit()` สลับลำดับ argument ผิดใน 8 จุดทั่วโปรเจกต์ (ดูรายละเอียดที่หัวข้อ 0.3 "บทเรียนที่ได้จากการทำงานรอบนี้" และรายการที่ 6–13 ในหัวข้อ "สิ่งที่เจอเพิ่มระหว่างแก้")
...
การลงมือแก้จริงเจอบั๊ก/ช่องโหว่เพิ่มอีก 14 จุดที่การอ่านโค้ดตอนแรกมองไม่เห็น (ต้องรันจริง/query live data/แก้ไฟล์ข้างเคียงถึงเจอ):
...
6. **Staff-chat function calling พังจริงบน production** — ผู้ใช้รายงานว่าถาม "ลูกค้ารายนี้เคยคืนกี่ครั้ง" แล้วบอทตอบ "เชื่อมต่อผู้ช่วย AI ไม่สำเร็จ" ตรวจ Vercel log เจอ Gemini API ตอบ 400 error "missing thought_signature" — สาเหตุคือโค้ดสร้าง `{functionCall:{name,args}}` เองแทนที่จะ echo `parts` เดิมของ model กลับไปตรงๆ ทำให้ `thought_signature` ที่ Gemini 3.x แนบมาหายไปตอน turn ถัดไป แก้โดย echo `parts` array ทั้งก้อนกลับ (`bf0de0a`) → ปิดแล้ว แต่ยังไม่ได้ยิงทดสอบซ้ำกับ Gemini API จริงหลังแก้ (ดู 0.2)
7. **`b2b_customers.customer_code` ไม่เคยถูกกรอกเลยสักราย** — ตั้งแต่ระบบเริ่มใช้งาน `reviewClient()` ตอนอนุมัติลูกค้าใหม่ insert แถวลง `b2b_customers` โดยไม่เคยเซ็ต `customer_code` เลย (ผู้ใช้พบเองว่าตารางว่างเปล่าในคอลัมน์นี้) แก้โดยบังคับให้ CSR กรอกรหัสลูกค้าในหน้า "ลูกค้ารออนุมัติ" ก่อนอนุมัติได้ ผ่าน field ใหม่ + validation ทั้งฝั่ง client และ server (`reviewClient()` throw ถ้าไม่มีค่า) (`71d7f96`) → ปิดแล้ว
8. **ลูกค้าลงทะเบียนสำเร็จแล้วค้างอยู่หน้าเดิม ไม่รู้ต้องทำอะไรต่อ** — เดิมใช้ `alert("ลงทะเบียนสำเร็จ!")` ปิด alert แล้วก็ยังค้างอยู่หน้าลงทะเบียนเดิม ไม่มีทางกลับหน้าหลัก แก้เป็น modal ยืนยันพร้อมข้อความ "รอดำเนินการใน 1-2 วัน" แล้ว auto-redirect กลับหน้า login หลังจากนั้น 2.2 วินาที (`9884936`) ทดสอบ responsive จริงตั้งแต่ 320px (มือถือจอเล็กสุด) ถึง desktop และแนวนอนจอเตี้ย ไม่มีปัญหา → ปิดแล้ว
9. **`checkRateLimit(key, limit, windowSeconds)` ถูกเรียกสลับลำดับ argument ผิดใน 8 จุดทั่วโปรเจกต์** — `auth.ts` (register), `send-pdf-email-action.ts`, `form-actions.ts`, `generate-pdf-action.ts` และ 4 จุดใน `staff-form-actions.ts` ทั้งหมดเรียกแบบ `(key, 3600, N)` แทนที่จะเป็น `(key, N, 3600)` ตาม JSDoc ที่ประกาศไว้จริง ผลคือ window จริงกลายเป็นแค่ไม่กี่วินาที (3–30 วิ) แทนที่จะเป็น 1 ชั่วโมงตามที่ตั้งใจ ทำให้ rate limit แทบไม่ทำงานจริงในจุดเหล่านี้มานาน (เจอตอนเขียน rate-limit ใหม่ให้ฟีเจอร์เอกสารยืนยันการลงทะเบียน แล้วสังเกตว่า argument order ไม่ตรงกับ JSDoc ของฟังก์ชัน) → ปิดแล้วครบทั้ง 8 จุด (5 จุดเดิม เช่น OTP กับ tracking เรียกถูกอยู่แล้ว)
10. **Double-submit race ใน `reviewClient()` ทำให้ insert ซ้ำเข้า `b2b_customers` จนชน unique constraint** — ผู้ใช้รายงานว่ากดอนุมัติแล้วเจอ error "duplicate key value violates unique constraint b2b_customers_email_key" ทั้งที่ Supabase ขึ้นว่าอนุมัติผ่าน สาเหตุ: อัปเดต `clients.status` โดยไม่เช็คว่ายังเป็น `'pending'` อยู่ไหม ทำให้คำขอซ้อนกัน (กดปุ่มซ้ำ/สองแท็บ) แข่งกัน insert ด้วยอีเมลเดียวกัน แก้โดยเปลี่ยน update เป็น `.eq('status','pending')` + เช็คว่ามีแถวถูกอัปเดตจริง (atomic compare-and-swap ระดับ DB) พร้อม disable ปุ่มฝั่ง UI ระหว่างรอผลด้วย → ปิดแล้ว
11. **ไม่ rollback `clients.status` เมื่อ `b2b_customers` insert ล้มเหลวจากสาเหตุอื่น** — แก้ข้อ 10 แล้วยังเจอซ้ำอีก เพราะเป็นสาเหตุคนละอย่าง: ลูกค้าลงทะเบียนซ้ำด้วยอีเมลเดิมหลังเคยอนุมัติไปแล้วรอบนึง (ลบ `clients` row เก่าไปแล้วแต่ `b2b_customers` เก็บถาวรไม่ลบตาม) ทำให้ insert ชน unique constraint จริงๆ ไม่ใช่แค่ race และเพราะ status update กับ insert เป็นคนละ statement ไม่ใช่ transaction เดียวกัน client จึงค้างที่ `'approved'` ทั้งที่ไม่มี `b2b_customer_id` ผูกอยู่จริง แก้โดยย้อนสถานะกลับ `'pending'` อัตโนมัติเมื่อ insert ล้มเหลว พร้อมข้อความ error ที่อ่านเข้าใจง่ายขึ้นสำหรับกรณีอีเมลซ้ำ → ปิดแล้ว
12. **`document_attachments` insert ของเอกสารยืนยันการลงทะเบียนไม่เคยเช็ค error เลย** — ทำให้ถ้า insert ล้มเหลว ไฟล์ PDF ที่เพิ่ง upload สำเร็จไปแล้วกลายเป็นไฟล์ลอยไม่มีทางเรียกดูได้อีก (เพราะ `getRegistrationDocumentUrl` ค้นหาผ่าน `document_attachments.client_id` เท่านั้น) โดยไม่มี log อะไรเลย → แก้ให้เช็ค error แล้ว throw ออกมาเพื่อให้เห็นใน log จริง
13. **DB trigger `check_document_attachment_consistency()` ไม่รู้จักคอลัมน์ `client_id` — insert เอกสารยืนยันการลงทะเบียนพังทุกครั้งไม่มีเว้น** — root cause ที่แท้จริงของข้อ 12: trigger นี้มีอยู่ก่อนฟีเจอร์นี้แล้ว บังคับว่า `request_id`/`ref_id` ห้าม null พร้อมกันทั้งคู่ (ออกแบบไว้สำหรับเอกสารแบบฟอร์มรับคืนอย่างเดียว) แต่แถวเอกสารยืนยันการลงทะเบียนมีแค่ `client_id`+`file_path` โดยตั้งใจ ทำให้ insert ชน trigger นี้ **100% ของครั้ง** ไม่ใช่บางครั้ง — PDF ขึ้น bucket ได้ตลอดเพราะ upload เป็นคนละ step และไม่เคยมีการเชื่อมโยงกลับมาได้เลยสักครั้งเดียวตั้งแต่ deploy ฟีเจอร์นี้ แก้โดยอัปเดต trigger ให้ยอมรับ `client_id` เป็นอีกทางเลือกได้ (migration `20260729020000`) พร้อม backfill แถวที่เคยพังไว้ก่อนหน้า → ปิดแล้ว
14. **ตรวจสอบด้วย `/anthropic-skills:nextjs-supabase-playbook` เจอ 2 จุดที่พึ่งพา default behavior แทนการล็อกสิทธิ์ชัดเจน** (ระดับ 🟡/🔵 ไม่ใช่ช่องโหว่ที่ใช้งานได้จริง แต่ไม่ explicit พอ): (ก) `request_pings` มีแค่ policy จำกัด `service_role` ไม่มี `deny_client_access` ให้ anon/authenticated แบบตารางอื่น (ปลอดภัยเพราะ RLS default-deny เมื่อไม่มี policy match แต่เสี่ยงถ้าใครปิด RLS โดยไม่รู้ว่าการป้องกันหายไปทันที) (ข) `check_document_attachment_consistency()` และ `sync_timeline()` (trigger function ทั้งคู่) ยังมี EXECUTE grant ให้ anon/authenticated เรียกตรงผ่าน RPC ได้ (ไม่มีผลจริงเพราะพึ่ง `NEW`/`OLD` context ที่มีแค่ตอน trigger เรียกจริง) แก้ทั้งคู่แล้ว (`20260729030000`) — ระหว่างแก้เจอเพิ่มว่า `REVOKE EXECUTE ... FROM PUBLIC` เฉยๆ **ไม่พอ** เพราะ Supabase grant EXECUTE ให้ anon/authenticated แบบ direct grant ตอนสร้างฟังก์ชันใหม่เสมอ (ไม่ใช่แค่สืบทอดผ่าน PUBLIC) ต้อง `REVOKE ... FROM anon, authenticated` ตรงๆ ถึงจะได้ผลจริง — ยืนยันด้วย `has_function_privilege()` แล้วว่าตอนนี้เป็น `false` ทั้งคู่

### สรุปตัวเลขรวม

**นับเฉพาะ finding เดิมในรายงานฉบับแรก:** 5/5 วิกฤต + 3/6 ปานกลาง + 2/3 เล็กน้อย (อีก 1 ปิดครึ่งเดียว) + 2/4 กลุ่ม performance = **12 จาก 18 จุดที่ระบุไว้ปิดแล้วเต็ม + 1 จุดปิดบางส่วน** — จุดวิกฤตทุกจุดปิดหมดแล้ว 100% ตั้งแต่รอบก่อน จุดที่เหลือ (5.7, 5.10, 5.11, เนื้อหาซ้ำ guide/manual, FK ไม่มี index, unused index) อยู่ในระดับ "ควรทำ" ทั้งหมด ไม่มีจุดไหนเป็น "ต้องทำก่อนใช้งานจริง" แล้ว บวกกับเจอและปิดช่องโหว่/บั๊กเพิ่มอีก 14 จุดที่ไม่เคยอยู่ใน list เดิมเลย (5 จุดเจอจากการทดสอบ flow อนุมัติลูกค้าจริงหลังส่งมอบฟีเจอร์เอกสารยืนยันการลงทะเบียน + 1 จุดล่าสุดเจอจากการตรวจสอบด้วย nextjs-supabase-playbook skill — ดูข้อ 9–14)
...
- **เอกสารยืนยันการลงทะเบียน (PDF + อีเมล):** ตอน CSR กรอกรหัสลูกค้าแล้วกดอนุมัติ ระบบสร้าง PDF อัตโนมัติ (ข้อมูลหน่วยงาน/ผู้ติดต่อชุดเดียวกับตอนลงทะเบียน, รหัสลูกค้า, ลายเซ็นลูกค้า, ส่วน "ส่วนของพนักงาน GPO" พร้อม checkbox อนุมัติ/ไม่อนุมัติ + ชื่อพนักงาน + วันที่ + ช่องเซ็นกำกับด้วยลายมือจริงบนกระดาษ) เก็บใน bucket ใหม่ `registration-documents` (private) ผูกกับตาราง `document_attachments` ที่มีอยู่แล้ว (เพิ่มคอลัมน์ `client_id` แทนสร้างตารางใหม่) ส่งอีเมลแจ้งลูกค้าพร้อมลิงก์ดาวน์โหลด และเพิ่มปุ่มดู/ดาวน์โหลดย้อนหลังในหน้า CSR ค้นหาลูกค้า — ใช้ pattern เดียวกับระบบ generate PDF แบบฟอร์มรับคืนที่มีอยู่แล้ว (`pdf-lib` + Resend) ออกแบบให้ generation/email ไม่บล็อกผลอนุมัติแม้ล้มเหลว แต่ระหว่างทดสอบจริงเจอบั๊กต่อเนื่อง 5 จุด (ข้อ 9–13 ด้านบน) กว่าจะใช้งานได้จริงสมบูรณ์

### 0.2 ยังไม่ได้ยืนยันแบบ live

**Staff-chat function calling แก้บั๊กแล้วแต่ยังไม่ได้ยิงเข้า Gemini API จริงซ้ำ** — เดิม (0.2 รอบที่แล้ว) เป็นความเสี่ยงเชิงทฤษฎีว่ายังไม่เคยทดสอบ runtime จริง ต่อมาผู้ใช้เจอ error จริงบน production ("เชื่อมต่อผู้ช่วย AI ไม่สำเร็จ") ยืนยันว่าเป็นบั๊กจริงตามที่กังวลไว้ (root cause: ไม่ได้ echo `thought_signature` กลับให้ Gemini 3.x) แก้แล้วใน `bf0de0a` (ดูรายละเอียดข้อ 6 ในหัวข้อ "สิ่งที่เจอเพิ่มระหว่างแก้") แต่ผู้ใช้ขอพักการไล่ verify ต่อ ("แก้ไขไปได้ประมาณนึงแล้ว เราพักส่วนนี้เอาไว้ก่อน") — ยังควรทดสอบถามคำถามที่ต้อง query ข้อมูลสด (เช่น "ลูกค้ารายนี้เคยคืนกี่ครั้ง") ผ่านแชทบอทพนักงานจริงอีกครั้งก่อนพึ่งพาฟีเจอร์นี้เต็มที่
...
**7. "ไม่บล็อกผล" กับ "ไม่เช็ค error เลย" เป็นคนละเรื่องกัน — ทำให้บั๊กที่พังทุกครั้งไม่มีเว้นแอบซ่อนอยู่ได้นาน**
ฟีเจอร์เอกสารยืนยันการลงทะเบียนออกแบบให้การสร้าง PDF/ส่งอีเมลเป็น side-effect ที่ไม่บล็อกผลการอนุมัติ (ถูกต้องตามที่ผู้ใช้ต้องการ) แต่ตอนเขียนจริงกลับไม่เช็ค error ของ `document_attachments.insert()` เลยด้วย ผลคือ insert ที่พัง **100% ของทุกครั้ง** (เพราะ DB trigger เก่าไม่รู้จักคอลัมน์ใหม่ — ดูข้อ 13) ไม่มีใครสังเกตเห็นเลยจนกว่าจะไปกดดูเอกสารจริงแล้วหาไม่เจอ ทั้งที่การอนุมัติ "ดูเหมือน" สำเร็จปกติทุกครั้ง — **บทเรียน: "non-blocking" ต้องคู่กับ "log/surface error เสมอ" (throw ให้ catch ชั้นนอกจับ แล้ว log จริง) ไม่ใช่แค่ไม่ throw เฉยๆ ไม่งั้นความล้มเหลวแบบ 100% จะดูเหมือนทำงานปกติได้เรื่อยๆ โดยไม่มีใครรู้**
