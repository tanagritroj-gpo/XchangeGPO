...
หลังจากประเมินครั้งแรก มีการแก้ไขเป็น commit บน branch `RLS_supabase` ต่อเนื่องรวม 20 ครั้ง (`9c47595` → `c082350`) สรุปสถานะเทียบกับ finding เดิมในเอกสารนี้ — รอบก่อนหน้า (`46467a7` → `a540fe7`) ปิด 5.8, 5.9 และ `console.log` ที่หลุดไป browser เพิ่ม พร้อมรวมเนื้อหานโยบายที่ซ้ำซ้อนเป็นแหล่งเดียว และเพิ่มความสามารถ AI chatbot ทั้งสองตัว ส่วนรอบล่าสุด (`bf0de0a` → `c082350`) แก้บั๊ก function calling ของ staff-chat, ปิดช่องโหว่ `b2b_customers.customer_code` ไม่เคยถูกกรอก และปิดจุด UX ที่ลูกค้าค้างอยู่หน้าลงทะเบียนหลังกดยืนยันสำเร็จ (ดูรายละเอียดที่หัวข้อ 0.3 "บทเรียนที่ได้จากการทำงานรอบนี้" และรายการที่ 6–8 ในหัวข้อ "สิ่งที่เจอเพิ่มระหว่างแก้")
...
การลงมือแก้จริงเจอบั๊ก/ช่องโหว่เพิ่มอีก 8 จุดที่การอ่านโค้ดตอนแรกมองไม่เห็น (ต้องรันจริง/query live data/แก้ไฟล์ข้างเคียงถึงเจอ):
