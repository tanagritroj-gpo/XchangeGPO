...
## 0. อัปเดตความคืบหน้า (ติดตามผล ณ 2026-07-28)

หลังจากประเมินครั้งแรก มีการแก้ไขเป็น commit บน branch `RLS_supabase` ต่อเนื่อง 6 ครั้ง (`9c47595` → `ce4eca3`) สรุปสถานะเทียบกับ finding เดิมในเอกสารนี้:
...
### 🟡 ระดับปานกลาง (หัวข้อ 5.6–5.11) — **ปิด 2/6**
...
| 5.8 | `lib/storage.ts` bucket public ไม่สอดคล้องกับ flow ที่ปลอดภัยกว่า | ⬜ ยังไม่แก้ (ยืนยันแล้วว่ายังอยู่เหมือนเดิม) |
| 5.9 | จับคู่สถานะด้วย regex ข้อความไทยแบบ free-text | ⬜ ยังไม่แก้ |
...
### 🟢 ระดับเล็กน้อย — **ปิด 1/3**

- ✅ `lib/supabase/config.ts` (dead code) — ลบแล้วพร้อม scaffold cleanup (`04de09d`)
- ⬜ `console.log` หลุดไป browser ใน `lib/supabase/client.ts:5` — ยังอยู่
- ⬜ เนื้อหาซ้ำ (chatbot-knowledge vs ReturnPolicySection, guide vs manual) — ยังไม่แก้
...
การลงมือแก้จริงเจอบั๊ก/ช่องโหว่เพิ่มอีก 4 จุดที่การอ่านโค้ดตอนแรกมองไม่เห็น (ต้องรันจริง/query live data ถึงเจอ):
...
**นับเฉพาะ finding เดิมในรายงานฉบับแรก:** 5/5 วิกฤต + 2/6 ปานกลาง + 1/3 เล็กน้อย + 2/4 กลุ่ม performance = **10 จาก 18 จุดที่ระบุไว้ปิดแล้ว** — และที่สำคัญที่สุดคือ **จุดวิกฤตทุกจุดปิดหมดแล้ว 100%** จุดที่เหลือทั้งหมดอยู่ในระดับ "ควรทำ" ไม่ใช่ "ต้องทำก่อนใช้งานจริง" อีกต่อไป บวกกับเจอและปิดช่องโหว่/บั๊กเพิ่มอีก 4 จุดที่ไม่เคยอยู่ใน list เดิมเลย
