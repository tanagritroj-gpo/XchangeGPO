...
// (getSaleCustomerHistory/getSaleStatusLogs แทน getCSRDashboardData/getManagerStatusLogs)
// และเกตสิทธิ์ (getSaleCoverage คืน null ถ้าไม่ใช่ sale หรือยังไม่ได้กำหนดขอบเขต — ใช้แทน
// getManagerOrCsrSession เพราะต้องเช็ค sale_provinces/org_type ด้วย ไม่ใช่แค่ department)
...
export async function GET(request: NextRequest) {
  ...
  const allRequests = (history ?? []) as ReportRequestRow[];
  ...
  const titleRow = summarySheet.addRow(['Sale Report Center']);
  ...
  requests.forEach((r) => {
    (r.drug_items || []).forEach((item: ReportDrugItemRow) => {
      drugSheet.addRow({
        ref_id: r.ref_id,
  ...
  return new NextResponse(buffer as unknown as BodyInit, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="sale-report-${dateStamp}.xlsx"`,
    },
  });
}
