...
const TAB_ACCENTS = {
  red: { bg: 'bg-destructive/10', text: 'text-destructive', bar: 'bg-destructive' },
  neutral: { bg: 'bg-accent', text: 'text-accent-foreground', bar: 'bg-primary' },
} as const;
...
function TabButton({ icon: Icon, label, count, active, onClick, accent }: {
  ...
  return (
    ...
    >
      {active && (
        <span className={`absolute left-0 top-2 bottom-2 w-[3px] rounded-full ${a.bar}`} />
      )}
      <span className={`w-8 h-8 rounded-md flex items-center justify-center shrink-0 transition-colors ${active ? a.bg : 'bg-secondary'}`}>
      ...
      {count !== undefined && (
        <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full shrink-0 tabular-nums ${active ? `${a.bg} ${a.text}` : 'bg-secondary text-muted-foreground'}`}>
          {count}
        </span>
      )}
    </button>
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-3 md:py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 md:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-sm md:text-base font-bold text-foreground leading-tight truncate">SLA Monitoring System</h1>
                        ...
                        <div key={rule.status_name} className="flex flex-col sm:flex-row sm:items-center gap-3 px-4 md:px-6 py-3.5">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              className="w-16 text-sm border border-border rounded-md px-2 py-1.5 tabular-nums"
                            />
                          </div>
                          <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            disabled={isSaving}
                            className="flex items-center gap-1.5 text-xs font-bold text-primary-foreground bg-primary hover:bg-primary/90 px-3.5 py-2 rounded-md transition-colors disabled:opacity-60 disabled:pointer-events-none sm:ml-auto"
                          >
