...
describe('authorization guard uses department, not role — unlike wh/logistics', () => {
  it('rejects a "manager role" session whose department is not csr or manager', async () => {
    ...
    mockGetStaffSession.mockResolvedValue({ id: 'x', username: 'x', full_name: 'Test Staff', department: 'wh', role: 'manager', sale_customer_types: null, sale_provinces: null, email: null, signature_url: null, mfa_enabled: false, mfa_grace_until: null });
